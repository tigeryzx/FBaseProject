...
namespace IFoxtec.Tests
{
    public abstract class IFoxtecTestBase : AbpIntegratedTestBase<IFoxtecTestModule>
    {
        ...
        protected void UsingDbContext(Action<IFoxtecDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<IFoxtecDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<IFoxtecDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<IFoxtecDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<IFoxtecDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IFoxtecDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<IFoxtecDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IFoxtecDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<IFoxtecDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<IFoxtecDbContext, Task<T>> func)
        {
            T result;
